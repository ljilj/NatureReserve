...
if ( ! function_exists( 'NatureReserve_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function NatureReserve_posted_on() {
		...
		$posted_on = sprintf(
			esc_html_x( 'Posted on %s', 'post date', 'NatureReserve' ),
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
		);

		$byline = sprintf(
			esc_html_x( 'by %s', 'post author', 'NatureReserve' ),
...
if ( ! function_exists( 'NatureReserve_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function NatureReserve_entry_footer() {
		// Hide category and tag text for pages.
		if ( 'post' == get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'NatureReserve' ) );
			if ( $categories_list ) {
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'NatureReserve' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'NatureReserve' ) );
			if ( $tags_list ) {
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'NatureReserve' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}

		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link">';
			comments_popup_link( esc_html__( 'Leave a comment', 'NatureReserve' ), esc_html__( '1 Comment', 'NatureReserve' ), esc_html__( '% Comments', 'NatureReserve' ) );
			echo '</span>';
		}

		edit_post_link( esc_html__( 'Edit', 'NatureReserve' ), '<span class="edit-link">', '</span>' );
	}
endif;

if ( ! function_exists( 'NatureReserve_post_navigation' ) ) :
	/**
	 * Lf Post Navigation
	 */
	function NatureReserve_post_navigation() {
		?>
		<div class="nav-links">
		<?php
		global $wp_query;
...
if ( ! function_exists( 'NatureReserve_share_links' ) ) :
	/**
	 * Lf Share links
	 **/

	function NatureReserve_share_links() {
		?>
...
if ( ! function_exists( 'NatureReserve_archive_title' ) ) :
	/**
	 * @param string $before_title
	 * @param string $after_title
	 * @return string
	 */
	function NatureReserve_archive_title( $before_title = "", $after_title = "" ) {
...
if ( ! function_exists( 'the_social_links' ) ) :
	...
	function the_social_links( $echo = false ){
		...
		$return_html = '<div class="lf-social-wrapper">';
			...
			if($twitter_icon!=''): $tw_icon ='<img src='.$twitter_icon.'>'; else: $tw_icon = '<i class="icon icon-Twitter-01"></i>';endif;
			if($google_icon!=''): $go_icon = '<img src='.$google_icon.'>'; else: $go_icon = '<i class="icon icon-google-plus-g"></i>';endif;
			if($youtube_icon!=''): $you_icon = '<img src='.$youtube_icon.'>'; else: $you_icon = '<i class="icon icon-Youtube-01"></i>';endif;
			if($linkedIn_icon!=''): $li_icon = '<img src='.$linkedIn_icon.'>'; else: $li_icon = '<i class="icon icon-Linkedin-01"></i>';endif;
			if($instagram_icon!=''): $inst_icon = '<img src='.$instagram_icon.'>'; else: $inst_icon ='<i class="icon icon-Instagram-01"></i>';endif;
