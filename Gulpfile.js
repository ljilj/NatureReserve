let project_name = "NatureReserve";
...
let config = (function () {
  return {
    ...
    /***********************************************************************
		...
		/***********************************************************************/
    text_domain: "" || project_name, // Example: 'NatureReserve' #lowercase, separated by underscore or dash ( _ or - )
    packageName: "" || project_name, // Example: 'NatureReserve package' #Capitalized

    flags: {
      production: !!gutil.env.production,
    },
    browserSyncOptions: {
      proxy: "https://localhost/" + project_name,
      open: true,
      injectChanges: true,
      ghostMode: false,
    },
...
function css() {
  return gulp
    ...
    .pipe(
      sass({
        outputStyle: !config.flags.production ? "expanded" : "compressed",
      })
    )
...
function plugins_js() {
  return gulp
    .src(["assets/js/plugins/*.js"], { allowEmpty: true })
...
function merge_js() {
  return gulp
    .src(["dist/plugins.js", "dist/site.js"], { allowEmpty: true })
    .pipe(plumber(config.plumberMessageError))
    .pipe(!config.flags.production ? sourcemaps.init() : gutil.noop())
...
function files(cb) {
  if (config.flags.production) {
    gulp
      .src(config.uploads_dir.files, {
        base: config.uploads_dir.files,
        allowEmpty: true,
      })
      .pipe(
        imagemin({
